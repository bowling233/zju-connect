...
func parseTOMLConfig(configFile string, conf *configs.Config) error {
	...
	conf.CaptchaServerBind = getTOMLVal(confTOML.CaptchaServerBind, "")
...
func init() {
	...
	flag.StringVar(&conf.CaptchaServerBind, "captcha-server-bind", "", "aTrust captcha server bind address (e.g. 0.0.0.0:40855). Random port on 127.0.0.1 if empty")
